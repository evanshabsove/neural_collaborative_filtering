...
from keras.layers import Embedding, Input, Dense, Flatten, Dropout, Concatenate
...
def get_model(num_users, num_items, layers=[20, 10], reg_layers=[0, 0]):
    ...
    for idx in range(1, num_layer):
        layer = Dense(layers[idx], kernel_regularizer=l2(reg_layers[idx]), activation='relu', name=f'layer{idx}')
        vector = layer(vector)
...
if __name__ == '__main__':
    ...
    model_out_file = f'Pretrain/{args.dataset}_MLP_{args.layers}_{int(time())}.weights.h5'
